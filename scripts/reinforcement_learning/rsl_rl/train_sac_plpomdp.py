...
"""Trains PL-POMDP's own (ported) SAC agent against a learned reward model on an IsaacLab task,
run as-is with no KL-anchor-to-reference mechanism. Structurally identical to
train_td3_plpomdp.py -- see that file's docstring for the shared rationale (Phase 2 of the
PL-POMDP migration roadmap, off-policy simplicity vs. the PPO port, RslRlVecEnvWrapper/
LearnedRewardWrapper reuse). Only the agent class and its SAC-specific hyperparameters differ
(alpha instead of act_noise/target_noise/noise_clip/policy_delay).

Usage:
    ./isaaclab.sh -p scripts/reinforcement_learning/rsl_rl/train_sac_plpomdp.py \\
...
parser = argparse.ArgumentParser(description="Train PL-POMDP's own SAC against a learned reward, no KL anchor.")
parser.add_argument("--task", type=str, required=True, help="Name of the task.")
parser.add_argument("--num_envs", type=int, default=1, help="Only num_envs=1 is supported this "
                     "phase -- vectorizing SAC is deferred to Phase 3.")
...
parser.add_argument("--alpha", type=float, default=0.2, help="Fixed entropy-regularization "
                     ...
                     "tuning, kept faithful rather than adding it.")
