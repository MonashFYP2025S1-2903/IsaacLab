...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    # --- JOINT LIMIT PRINT (temporary, not part of normal train.py) ---
    robot = env.unwrapped.scene["robot"]
    print("JOINT_LIMITS_MARKER_START")
    print(f"TASK={args_cli.task}")
    print(f"joint_names={robot.data.joint_names}")
    print(f"joint_pos_limits_lower={robot.data.joint_pos_limits[0, :, 0].cpu().tolist()}")
    print(f"joint_pos_limits_upper={robot.data.joint_pos_limits[0, :, 1].cpu().tolist()}")
    print(f"default_joint_pos={robot.data.default_joint_pos[0].cpu().tolist()}")
    print(f"joint_effort_limits={robot.data.joint_effort_limits[0].cpu().tolist()}")
    print(f"joint_vel_limits={robot.data.joint_vel_limits[0].cpu().tolist()}")
    print("JOINT_LIMITS_MARKER_END")
    env.close()
    return
    # --- END JOINT LIMIT PRINT ---
